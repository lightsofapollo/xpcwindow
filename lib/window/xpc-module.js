...
window.xpcModule = (function() {
  ...
  /**
  Require conditions we need to meet.

  1. We can expose 'exports' which is available
     to the required file that can then be referenced
     after loading it via subscript loader

  2. A module can modify global state when wanted.
     There should be some option for both browser & node code.

  3. modules know where they are so they can load other modules
     using relative paths

  4. require should be oriented to xpcwindow or nodejs code.
  */
  ...
  Module.prototype = {
    ...
    require: function(path) {
      ...
      if (fsPath) {
        handler = fsPath.extname(path) || '.js';
      }
